...
class TestSearchHandlerDaysParameter:
    ...
    async def test_passes_none_days_when_not_provided(self, mock_dependencies, mock_orchestrator):
    ...
    async def test_passes_explicit_days_to_orchestrator(self, mock_dependencies, mock_orchestrator):
    ...
    async def test_displays_default_date_range_message_when_days_none(
        self, mock_dependencies, mock_orchestrator
    ):
        ...
        assert any(
            "default" in text.lower() or "using" in text.lower() for text in message._reply_texts
        ), f"Expected default date range indicator in messages: {message._reply_texts}"

    async def test_displays_explicit_days_in_message(self, mock_dependencies, mock_orchestrator):
        ...
        message = MockMessage(user=user)
        update = MockUpdate(user=user, message=message)
        context = MockContext(args=["days=10"], dependencies=mock_dependencies)

        await search_jobs_handler(update, context)

        # Should show "Last 10 days" or similar
        assert any(
            "10" in text and "days" in text.lower() for text in message._reply_texts
        ), f"Expected '10 days' in messages: {message._reply_texts}"
